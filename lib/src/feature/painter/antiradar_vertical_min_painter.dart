...
/// {@template antiradar_vertical_min_painter}
/// AntiradarVerticalMinPainter.
/// {@endtemplate}
class AntiradarVerticalMinPainter extends CustomPainter {
  /// {@macro antiradar_vertical_min_painter}

  @override
  void paint(Canvas canvas, Size size) {
    final firstCircleDxPosition = size.width / 2;
    final firstCircleDyPosition = size.height / 1.47;
    var paint = Paint()
      ..color = const Color(0xFFC9DBE4)
      ..strokeWidth = 2.0
      ..style = PaintingStyle.stroke;

    final firstCircleRadius = size.height * 0.18;
    ...
    canvas.drawArc(
        Rect.fromCircle(
            center: Offset(size.width / 2, size.height / 1.40), radius: radius),
        math.pi,
        math.pi,
        false,
        paint);
    ...
    final double centerY = size.height / 1.1;
    ...
    trianglePath.lineTo(size.width, size.height * 0.26);
    trianglePath.moveTo(size.width / 2, size.height * 0.73);
    trianglePath.lineTo(0, size.height * 0.26);
